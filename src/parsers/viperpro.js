...
async function* getData(catalogueURL, source) {
	for await (const product of source(catalogueURL)) {
		const { uid, url, title: name, sku: skuRaw, price, text, characteristics, gallery } = product;
		const options = getTabsOptions(catalogueURL, uid);
		const sku = getSKU(skuRaw, url, name);
		const category = f.formatCategory(CATALOGUE, url, CACHE.CURRENT.category);
		...
		try {
			...
			const description = f.formatDescription(getDescription(text), getDescriptionVideo(tabs));
			const properties = getProperties(characteristics);
			const docs = getDocs(tabs);
			const imagesfileNames = m.downloadImages(getImages(gallery), sku);
...
function getProductsListOptions(url, categoryFilter) {
	return {
		...
		params: { storepartuid: '491553257541', 'filters[storepartuid]': categoryFilter },
		headers: {
...
function getDescription(text) {
	const description = text
		...
		?.replace(/<strong[^>]*?>\s*КОМПЛЕКТАЦИЯ\s*<\/strong>/gi, '<h2>Комплектация</h2>')
		?.replace(/<strong[^>]*?>\s*ОСОБЕННОСТИ\s*<\/strong>/gi, '<h2>Особенности</h2>')
		|| '';
...
function getCategoryFullName(category) {
	return CATALOGUE.categories.get(category)
		|| category;
}