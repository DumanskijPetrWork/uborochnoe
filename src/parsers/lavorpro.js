...
async function* getRelatedData(source) {
	for (const cardURL of source) {
		...
		try {
			...
			const price = f.formatPrice(getPrice($));
			const description = f.formatDescription(getDescription($));
			const images = downloadImages([getImages($).at(0)], sku);
			const relatedAccessoriesSKUs = getRelatedAccessoriesSKUs(
				$,
			...
			console.error(
				`Ошибка ${getRelatedData.name} (url: ${cardURL}): ${e}`
			);
		}
	}
}
...
function getCategory($) {
	const category = $("div.breadcrumb a")?.eq(-1)?.text()?.trim() || "";
...
function getRelatedAccessories($) {
	return $("div.catalog_greed.notindex")?.find("div.item span.descr");
}
...
function addRelatedItems($, relatedAccessories) {
	getRelatedAccessoriesURLs($, relatedAccessories).forEach((url) =>
		CACHE.relatedItems.add(ORIGIN_URL + url)
	);
}
